...
float StarField::PrefStars = 1.0f;
//--------------------------------------------------------------------------- 
...
IMPLEMENT_PERSISTENT_TAG(StarField, StarFieldPersTag);

//--------------------------------------------------------------------------- 

void StarField::toggleGlobeLines()
{
...
void StarField::inspectWrite(Inspect *inspect)
{
...
void StarField::inspectRead(Inspect *inspect)
{
...
bool StarField::processQuery(SimQuery *query)
{
   switch (query->type) {
...
DWORD StarField::packUpdate(Net::GhostManager *, DWORD, BitStream *stream)
{
...
void StarField::unpackUpdate(Net::GhostManager *, BitStream *stream)
{
...
void StarField::initPersistFields()
{
   //Parent::initPersistFields();
   addField("inFrontOfSky", TypeBool, Offset(inFrontOfSky,StarField));
   addField("colors", TypePoint3F, Offset(colors, StarField), 3);
}

Persistent::Base::Error 
StarField::read(StreamIO &sio, int version, int u)
{
...
Persistent::Base::Error 
StarField::write(StreamIO &sio, int version, int u)
{
...
//--------------------------------------------------------------------------- 
void StarField::setBottomVisible(bool _visible)
{
...
//--------------------------------------------------------------------------- 
void StarField::setVisibility(bool _visible)
{
   visible = _visible;
   setMaskBits(Modified);
}
...
void StarField::init(GFXPalette *palette)
{
   if (setCurrentDistance() == false)
      return;
   ...
   if (globeLines)
   {
      for (float az_ = 0.0f; az_ < 360.0f; az_ += 15.0f)
         for (float inc_ = -90.0f; inc_ < 90.0f; inc_ += 2.0f) {
            ...
            c    = m_currentDistance*m_cos(inc);
            float b = 3;
            regions[r].stars.push_back(Star(Point3F(c*m_cos(az), 
                                                    c*m_sin(az), 
                                                    m_currentDistance*m_sin(inc)), b));
         }
   ...
   for (int inc = 0; inc < 90; inc += INC_REGION_SIZE) 
      for (int az = 0; az < 360; az += AZ_REGION_SIZE) {
         ...
         c = m_currentDistance*m_cos(DEGRAD*inc);
         point.pos = Point3F(c*m_cos(DEGRAD*az), 
                             c*m_sin(DEGRAD*az), 
                             m_currentDistance*m_sin(DEGRAD*inc));
         controlPoints[regionNo++] = point;         
      }

   // create a special case control point for the celestial north pole
   c   = m_currentDistance*m_cos(DEGRAD*90);
   point.pos = Point3F(c*m_cos(DEGRAD*0), c*m_sin(DEGRAD*0), m_currentDistance*m_sin(DEGRAD*90));
...
void StarField::render(TSRenderContext &rc)
{
   ...
   if (terrain)
      visibleDistance = terrain->getVisibleDistance() * 1.05f;
   else
...
//   displacement.z *= -(STARFIELD_DISTANCE - visibleDistance)/visibleDistance;
   ...
   else 
   {
      ...
      if (hFov > DEGRAD*(float(AZ_REGION_SIZE)) && vFov > DEGRAD*(float(INC_REGION_SIZE)))
      {
         ...
         if (n == rowMin) 
            regions[rowMax].visible = true;
         else 
            regions[n - 1].visible = true;
         if (n == rowMax)
            regions[rowMin].visible = true;
         else
            regions[n + 1].visible = true;
         ...
         if (row != Rows -1) {
            ...
            if (n == rowMin) 
               regions[rowMax + AZ_REGION_STRIDE].visible = true;
            else 
               regions[(n - 1) + AZ_REGION_STRIDE].visible = true;
            if (n == rowMax)
               regions[rowMin + AZ_REGION_STRIDE].visible = true;
            else
               regions[n + 1 + AZ_REGION_STRIDE].visible = true;
         } 
         if (row != 0) {
...
bool
StarField::setCurrentDistance()
{
   AssertFatal(manager != NULL, "Must have manager!");

   SimTerrain* pTerrain = (SimTerrain*)manager->findObject(SimTerrainId);
   if (pTerrain == NULL)
      return false;

   m_currentDistance = pTerrain->getVisibleDistance() * 1.05f;
