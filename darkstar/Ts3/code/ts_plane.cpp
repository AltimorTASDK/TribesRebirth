...
float TSPlaneF::DistancePrecision = TSPlaneDefaultPrecision;
float TSPlaneF::NormalPrecision = TSPlaneDefaultPrecision;
...
void TSPlaneF::set( const Point3F& k, const Point3F& j, const Point3F& l )
{
	Point3F	kj,lj,pv;
...
//
void TSPlaneF::set( int count, Point3F ** vl)
{
	Point3F normal;
	normal.x = normal.y = normal.z = float(0);
...
Bool TSPlaneF::intersect( const Point3F& start, const Point3F& end, float* value )
{
	Point3F dt = end;
	dt -= start;
	...
	if ((den = x * dt.x + y * dt.y + z * dt.z) == 0)
		return 0;
	float t = -(x * start.x + y * start.y + z * start.z + d);
...
Bool TSPlaneF::intersect( const LineSeg3F& line, Point3F* ip )
{
	Point3F	dt;
	float		t,den;
...
Bool TSPlaneF::split( const Poly3F& poly, Poly3F* front, Poly3F* back )
{
	LineSeg3F line;
