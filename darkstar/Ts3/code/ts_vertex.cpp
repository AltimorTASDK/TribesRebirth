...
{
   //---------------------------------------------------------------------------
   Point3F    PackedVertex::fNormalTable[] =
      {
   ...
   void TransformedVertex::calcLighting( RenderContext &rc, 
      Vertex const &vert )
      {
      if( !(fStatus & Lit) )
         {
         rc.getLights()->calcIntensity( vert, &fColor.color );
         fStatus |= Lit;
         }
      }
   ...
   UInt8 PackedVertex::encodeNormal( Point3F const &norm )
      {
      int   bestIndex = 0;
      float bestErr = 999999.0f;
      for( int i = 0; i < sizeof(fNormalTable)/sizeof(Point3F); i++ )
         {
         float newErr = m_dist( norm, fNormalTable[i] );
         if( newErr < bestErr )
            {
            bestErr = newErr;
            bestIndex = i;
