...
void MyCanvas::onCommand(int id, HWND hWndCtl, UINT codeNotify)
{
   ...
   switch (id)
   {
      ...
      default:
         ...
         console->printf( "%f", SimSky::PrefSkyDetail );
         SimObjects::dummy_link2 = 0;
         break;
   }
}
...
bool CMain::initInstance()
{
   ...
   FileRStream showCS("show2.cs");
   if(showCS.getStatus() == STRM_OK)
   {
      int      size = showCS.getSize();
      BYTE     * buffer = new BYTE[size+1];
      
      showCS.read( size, buffer );
      buffer[size] = 0;
      Console->evaluate((const char *)buffer, false, "show.cs");
      delete[] buffer;
   }
