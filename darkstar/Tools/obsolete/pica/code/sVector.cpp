...
extern "C" {
   ...
   asmCalcCenterOfMass(const quantVector* pCenterOfMass,
                       const sortVector*  pVectors,
                             ...
                             double       totalWeight);
   void __cdecl
   asmCalcCovarMatrix(const double*     pCenterOfMass,
                      double**          pCaovarMatrix,
...
double getStandardDeviation(const quantTableEntry* in_pQuantEntry);
...
void   calcCovarMatrix(const quantTableEntry* in_pQuantEntry,
                       double**               out_pCovarMatrix);
void   calcPrincipleEigenVector(const double** in_pMatrix,
                                double*        out_pEigenVector,
                                const int      in_dimension);
void   formDotProductKey(quantTableEntry* io_pQuantEntry,
                         const double*    in_pEigenVector);
...
getStandardDeviation(const quantTableEntry *in_pQuantEntry)
{
   ...
   centerOfMass.pElem  = new double[centerOfMass.numDim];
   
   calculateCenterOfMass(in_pQuantEntry, &centerOfMass);
   double standardDev = ::asmGetStandardDev(&centerOfMass,
...
sortQuantEntry(quantTableEntry *io_pQuantEntry)
{
   ...
   double** covarMatrix = new double* [dimension];
   for (i = 0; i < dimension; i++) {
      covarMatrix[i] = new double[dimension];
   }
   ...
   calcPrincipleEigenVector((const double **)covarMatrix, eigenVector, dimension);
...
void
calcPrincipleEigenVector(const double **in_pMatrix,
                               double  *out_pEigenVector,
                         const int      in_dimension)
{
   // Sanity checks
   ...
   double *tempVector1  = new double[in_dimension];
   double *tempVector2  = new double[in_dimension];
                      ...
                      double          **out_pCovarMatrix)
{
   ...
   centerOfMass.pElem  = new double[dimension];
