...
MPMFileList::~MPMFileList()
{
   for (int i = 0; i < m_fileEntries.size(); i++) {
      ...
      delete m_fileEntries[i].pRawPalette;
      m_fileEntries[i].pRawPalette = NULL;
   }
   m_fileEntries.clear();
}
...
void
MPMFileList::processFileString(const char* in_pFileLine)
{
   ...
   if (strchr(lineProper, '*') != NULL ||
      ...
      if (winHandle != INVALID_HANDLE_VALUE) {
         ...
         MPMFileEntry& firstEntry = m_fileEntries.last();
