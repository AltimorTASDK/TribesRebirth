...
//global instance of the resource manager and volume stream
ResourceManager	rm;
VolumeRStream	vrStream;
CString pSearchPath="";
...
//	rm.add( &vrStream );
	...
	ghAccelTable= LoadAccelerators(AfxGetInstanceHandle(), MAKEINTRESOURCE(ID_ACCELERATOR));
...
BOOL CMatilda2App::ProcessMessageFilter(int code, LPMSG lpMsg)
{
     if (code < 0)
