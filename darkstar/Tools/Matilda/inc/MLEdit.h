...
#define ValidMaterial(m) ((m) == TS::Material::MatNull || (m) == TS::Material::MatPalette || (m) == TS::Material::MatRGB || (m) == TS::Material::MatTexture)
#define ValidLighting(lt) ((lt) == TS::Material::ShadingNone ||(lt) == TS::Material::ShadingFlat ||(lt) == TS::Material::ShadingSmooth)
#define ValidTexture(tt) ((tt) == TS::Material::TextureTransparent ||(tt) == TS::Material::TextureTranslucent)
...
// ------------
	...
	TS::MaterialList* m_pmlCurr;

	// save last copy of MaterialList (for undo or cancel)
	TS::MaterialList* m_pmlOld;
...
// -----------------------------------------------------------
	...
	// handle textures
	void setMatToTexture(TS::Material::ShadingType ltKind, int iTextureFlags = (int)(TS::Material::TextureTransparent)); //TexturePlain
 	int getTextureFlags() const;

	// get pointer to current material
	TS::Material* getCurrMat();

	// get material type of current material: null, rgb, palette, texture
	TS::Material::MatType getMatType() const;

	// get the lighting type of the material (rgb, palette, texture)
	TS::Material::ShadingType getMatLighting() const;
	...
	void setMatToCommonProps ( const CommonDmlProperties & commProps );
	bool getCommonProps ( CommonDmlProperties & commProps );

	// Palette type of material
	void setMatToPalette(TS::Material::ShadingType ltKind, Int32 iPalIndex);
	int getPaletteIndex() const;

	// deal w/ RGB type
	void setMatToRGB(TS::Material::ShadingType ltKind, const WORD wRed, const WORD wGreen, const WORD wBlue);
	...
	void copyToCurrIndex(TS::Material *in_newMat);
