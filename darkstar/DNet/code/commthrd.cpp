...
BOOL StartComm(HANDLE hNewCommFile)
{
    ...
    {
        MessageString("File handle is not a comm handle.\n");
        return FALSE;
    }
    ...
    if (g_hCommFile != NULL)
    {
        MessageString("Already have a comm file open\n");
        return FALSE;
    }
    ...
    {
        MessageError(GetLastError(), "Unable to CreateEvent: ");
        g_hCommFile = NULL;
        return FALSE;
    }
    ...
    {
        MessageError(GetLastError(),"Unable to create Read thread");
    ...
    {
        MessageError(GetLastError(),"Unable to create Write thread");
...
void StopComm()
{
    ...
    MessageString("Stopping the Comm\n");
...
BOOL WriteCommString(LPCSTR lpszStringToWrite, DWORD dwSizeofStringToWrite)
{
    if (g_hWriteThread)
    {
                ...
                (WPARAM) dwSizeofStringToWrite, (LPARAM) lpszStringToWrite))
        {
            ...
            MessageString("Failed to Post to Write thread.\n");
    }
    else
        MessageString("Write thread not created\n");
...
void CloseReadThread()
{
    ...
    if (g_hReadThread)
    {
        MessageString("Closing Read Thread\n");
        ...
        {
            MessageString("Read thread not exiting.  Terminating it.\n");
...
void CloseWriteThread()
{
    ...
    if (g_hWriteThread)
    {
        MessageString("Closing Write Thread\n");
        ...
        {
            MessageString("Write thread not exiting.  Terminating it.\n");
...
DWORD WINAPI StartWriteThreadProc(LPVOID lpvParam)
{
    ...
    while (TRUE)
    {
        if (!PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
        {
            ...
            switch(dwHandleSignaled)
            {
                ...
                {
                    MessageError(GetLastError(),"Write WAIT_FAILED: ");
                    PostHangupCall();
                    goto EndWriteThread;
                }
                ...
                {
                    MessageError(dwHandleSignaled,"Unexpected Wait return value '%lx'");
                    PostHangupCall();
                    goto EndWriteThread;
                }
        ...
        switch(msg.message)
        {
            ...
            {
                MessageString("Writing to comm port\n");
  ...
  EndWriteThread:
    ...
    MessageString("Write thread shutting down\n");
    ...
    LPCSTR lpszStringToWrite, DWORD dwNumberOfBytesToWrite)
{
    ...
    do
    {
                ...
                lpOverlappedWrite))
        {
            ...
            {
                MessageString("ERROR_INVALID_HANDLE, "
            ...
            {
                MessageError(dwLastError,
                    "Error to writing to CommFile");
                
                MessageString("Closing TAPI\n");
                PostHangupCall();
                return FALSE;
            }
...
DWORD WINAPI StartReadThreadProc(LPVOID lpvParam)
{
    ...
    {
        MessageError(GetLastError(),"Unable to SetCommMask: ");
        PostHangupCall();
        goto EndReadThread;
    }
    ...
    while (TRUE)
    {
        ...
        switch(dwHandleSignaled)
        {
            ...
            case WAIT_FAILED:       // Wait failed.  Shouldn't happen.
            {
                MessageError(GetLastError(),"Read WAIT_FAILED: ");
                PostHangupCall();
                goto EndReadThread;
            }
  ...
  EndReadThread:
    ...
    MessageString("Read thread shutting down\n");
    PurgeComm(g_hCommFile, PURGE_RXABORT | PURGE_RXCLEAR);
    ...
    LPDWORD lpnNumberOfBytesRead)
{
  ...
  StartSetupReadEvent:
            ...
            lpnNumberOfBytesRead, lpOverlappedRead))
    {
        ...
        MessageString("Data waiting for ReadFile.\n");
    ...
    if (dwLastError == ERROR_IO_PENDING)
    {
        MessageString("Waiting for data from comm connection.\n");
        return TRUE;
    }
    ...
    MessageError(dwLastError,"Unexpected ReadFile error: ");
...
BOOL HandleReadData(LPCSTR lpszInputBuffer, DWORD dwSizeofBuffer)
{
    ...
    if (dwSizeofBuffer)
    {
        ...
        MessageString("Got something from Comm port!!!\n");

        lpszPostedBytes = (char *)LocalAlloc(LPTR,dwSizeofBuffer+1);
        if (lpszPostedBytes == NULL)
        {
            MessageError(GetLastError(), "LocalAlloc: ");
            return FALSE;
        }
        ...
        return PostCommData(lpszPostedBytes, dwSizeofBuffer);
    }
    ...
    LPDWORD lpfdwEvtMask)
{
  ...
  StartSetupCommEvent:
    ...
    if (WaitCommEvent(g_hCommFile, lpfdwEvtMask, lpOverlappedCommEvent))
    {
        ...
        MessageString("Event (Error) waiting before WaitCommEvent.\n");
    ...
    if (dwLastError == ERROR_IO_PENDING)
    {
        MessageString("Waiting for a CommEvent (Error) to occur.\n");
        return TRUE;
    }
    ...
    // Unexpected error. No idea what could cause this to happen.
    MessageError(dwLastError, "Unexpected WaitCommEvent error: ");
    return FALSE;
}
    ...
    LPDWORD lpfdwEvtMask, BOOL fRetrieveEvent)
{
    ...
    if (*lpfdwEvtMask & EV_ERR)
    {
        ...
        if (!ClearCommError(g_hCommFile, &dwErrors, NULL))
        {
            ...
            MessageError(GetLastError(),"ClearCommError: ");
            return FALSE;
        }
    ...
    MessageError((long)lpfdwEvtMask,"Unexpected comm event %lx");
    return FALSE;
}

//
//  FUNCTION: PostCommData(LPSTR, DWORD)
//
//  PURPOSE: Sends data to main thread
//
//  PARAMETERS:
//    lpNewString       - The data string
...
BOOL PostCommData(LPSTR lpNewString, DWORD dwSizeofNewString)
{
	MessageString("Data received...");
   MessageString(lpNewString);
    return TRUE;
}
