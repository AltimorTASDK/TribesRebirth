...
public:
   ...
   void registerClass(UINT style,HICON = 0,HCURSOR = 0,HBRUSH = 0, WNDPROC wndProc= DefWindowProc);
	const char* getName();
   void setName( const char *name );

	//
	static GWWindowClass* defaultClass();
   static GWWindowClass* defaultMDIChildClass();
};
...
inline void GWWindowClass::setName( const char *name )
{
   delete [] className;
   className = strnew(name);
}   
...
protected:
   ...
   bool useMessageMap;
   bool guaranteeRequestedClientSize;
   bool centerBloatedWindow;
   ...
   LRESULT DispatchMsg(const gwMsgMapEntry* in_pEntry,
...
public:
   ...
   bool getPrefPosition(LPCSTR wname, RECT *rect);
   ...
   bool create(GWWindowClass* wclass, LPCSTR wname, DWORD style,
                       ...
                       bool usePrefPos = FALSE, bool usePrefSize = FALSE);
   
	bool createEx(DWORD exstyle, GWWindowClass* wclass,LPCSTR wname, DWORD style,
		const Point2I pos,const Point2I size, GWWindow* parent,int id,HMENU hMenu=NULL,
		bool usePrefPos = FALSE, bool usePrefSize = FALSE);   //for extended styles

	bool createMDIChild(GWWindowClass* wclass,LPCSTR wname, DWORD style,
		const Point2I pos,const Point2I size, GWWindow* parent,int id);

	bool createMDIClient(LPCSTR wname, DWORD style,
	    const Point2I pos,const Point2I size, GWWindow* parent,int id,
   ...
   void setMinSize(Point2I size);
   void setMaxSize(Point2I size);

   void guaranteeReqClientSize(bool tf) {guaranteeRequestedClientSize = tf;}
   // if the window is bigger then the screen, center the window so
   // that the center of the client area is the center of the screen
   void centerBloatedWin(bool tf) {centerBloatedWindow = tf;}
   ...
   // Windows events
   virtual void  onContextMenu( int x, int y, HWND hwnd );
   ...
   virtual void  onDisplayChange(UINT cBitsPerPixel, int cx, int cy);
   ...
   virtual void   onDeviceChange(WPARAM, LPARAM);
   virtual void   onCommNotify(int cid, UINT flags);
   virtual int    onNotify(int id, LPNMHDR pnmhdr);
   virtual void   onMM_MCINotify(WPARAM wFlags, LPARAM lDevID);
   virtual void   onMM_MIXMControlChange(WPARAM hMixer, LPARAM dwControlId);
   virtual void   onMM_MIXMLineChange(WPARAM hMixer, LPARAM dwLineId);
...
inline void GWWindow::quitOnDestroy(bool b)
{
   postQuit = b;
}   
